...
for t=0:dt:T
    ...
    [cost,cost_nabla,cost_nabla_dot,cost_hessian] = quadcost([pb_hat';th'],t,Acost,bcost,dAcost,dbcost);
    % barrier
    Ath = [0,1,0;0,-1,0;0,0,1;0,0,-1];
    bth = [pi*0.9;0;0;pi*0.9];
    Abarr = blkdiag(Areg,Ath);
    bbarr = [breg;bth];
    [bar,bar_nabla,bar_nabla_dot,bar_hessian] = logbarrier([pb_hat';th'],t,Abarr,bbarr);
    % cost+bar
    if imag(bar)
    ...
    dot_state(1,:) = -hessian_inv*phi;
    dot_pb_hat = dot_state(1:2);
    dmu = dot_state(3:end);
    % th optimization
